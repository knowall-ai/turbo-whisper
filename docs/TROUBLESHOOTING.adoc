...
|**Focused window detection shows just "Listening" on KDE Wayland**
|Wayland's security model restricts apps from querying other windows. On KDE Plasma Wayland, install `kdotool` (`yay -S kdotool` on Arch, or `cargo install kdotool`). Turbo Whisper will automatically detect `kdotool` and display the active application name next to "Listening".
