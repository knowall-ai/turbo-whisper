...
|Install ydotool: `sudo apt install ydotool`. Add user to input group: `sudo usermod -aG input $USER`. Log out and back in for the group change to take effect.

|**ydotool error: "failed to open uinput device"**
|ydotool needs permission to access `/dev/uinput`. Run: `sudo usermod -aG input $USER` then log out and back in. Alternatively, run the app with sudo for testing (not recommended for regular use).

|**wtype error: "Compositor does not support virtual keyboard protocol"**
|KDE Plasma doesn't support wtype. Use ydotool instead, or rely on xdotool for XWayland apps.

|**xdotool types into wrong window**
|Increase the delay before typing. The app window might not have fully hidden before xdotool starts. Edit typer.py to increase `time.sleep()` value.
